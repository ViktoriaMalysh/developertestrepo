...
export default async function LocationPoliciesPage({ params }: PageProps) {
  ...
  const [websiteData] = await Promise.all([getWebsiteBySlug(slug)]);
        ...
        <div className="container mx-auto px-4 py-4 max-w-screen-2xl">
          ...
          >
            {pageMetadata.hero_subheading ||
              "Browse our insurance policies - auto, home, life, and business coverage. Compare options and get personalized quotes today."}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {allPolicies.map((policy) => (
            <Link
              ...
              className="block"
            >
              <div className="bg-card-bg rounded-xl p-8 text-center shadow-lg border border-[var(--divider-color)] hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 cursor-pointer h-full">
