...
export async function generateMetadata({
  ...
  const pageMetadata = await getPageMetadata(
    "blog",
    websiteData?.client_locations?.location_id
  );
...
export default async function LocationBlogPage({ params }: PageProps) {
  ...
  const topics = await getAllTopics(slug, {
    agency_name: websiteData?.agency_name,
    city: websiteData?.client_locations?.city,
    state: websiteData?.client_locations?.state,
  });
  ...
  const locationCity =
    websiteData?.client_locations?.city || clientData?.city || "";
  const locationState =
    websiteData?.client_locations?.state || clientData?.state || "";
  const agencyName = clientData?.agency_name || "";
    ...
    <main className="flex-grow">
      ...
      >
        <div className="container mx-auto px-4 py-4 max-w-screen-2xl">
          ...
          >
            {locationCity || ""}
            {locationState ? ` ${locationState},` : ""} Insurance Blog
          </h1>
      ...
      <section className="py-16 bg-[#eae0d5]">
        <div className="container mx-auto px-4 max-w-screen-xl text-center">
          ...
          <p className="text-theme-body mb-8 max-w-2xl mx-auto">
            Get the latest insurance tips, industry news, and special offers
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(
            buildLdJsonSchema(clientData, websiteData, slug)
          ),
        }}
      />
    </main>
