...
export async function generateMetadata({
  ...
  return {
    ...
    description:
      ...
      `Learn more about ${agencyName} at ${locationName}.`,
...
async function getAboutPageData(locationId: string | null): Promise<{
  ...
  const { data, error } = await supabase
    ...
    .select(
      "hero_section, intro_section, legacy_section, policies_section, community_section, local_knowledge_section, personal_approach_section, thank_you_section, cta_section"
    )
...
export default async function LocationAboutPage({ params }: PageProps) {
  ...
  const ldJsonSchema = buildLdJsonSchema(
    clientData,
    businessInfo,
    websiteData,
    slug
                    ...
                    <div
                      className={
                        introSection?.image?.url
                          ? "grid grid-cols-1 md:grid-cols-2 gap-10 items-center mb-12"
                          : "mb-12"
                      }
                    >
                      <div>
                        <div className="text-theme-body">
                          {introSection?.description?.paragraphs
                            ? Object.keys(introSection.description.paragraphs)
                                .sort()
                                .map((key) => {
                                  const paragraph =
                                    introSection.description.paragraphs[key];
                                  return paragraph ? (
                  ...
                  <div className="bg-theme-bg/30 rounded-lg p-8 mb-12">
                    <div className="flex flex-col md:flex-row items-start gap-6">
                      ...
                      <div>
                        ...
                        <div className="text-theme-body">
                          {legacySection?.description?.paragraphs
                            ? Object.keys(legacySection.description.paragraphs)
                                .sort()
                                .map((key) => {
                                  const paragraph =
                                    legacySection.description.paragraphs[key];
                                  return paragraph ? (
                                    <p key={key} className="mb-4 last:mb-0">
                                      {paragraph?.content || ""}
                                    </p>
                  ...
                  <>
                    ...
                    <div className="text-theme-body mb-8 text-center max-w-3xl mx-auto">
                      {policiesSection?.description && (
                        <p>{policiesSection.description?.content || ""}</p>
                      )}
                    ...
                    {policiesSection?.insurance_types?.items &&
                      policiesSection.insurance_types.items.length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto mb-8">
                          {policiesSection.insurance_types.items.map(
                            (item, index) => (
                              <Link
                                key={index}
                                href={`/locations/${slug}/policies/${item?.link || ""}`}
                                className="block"
                              >
                    ...
                    {policiesSection?.insurance_note && (
                      <div className="text-theme-body mb-12 text-center max-w-3xl mx-auto">
                        <p>{policiesSection.insurance_note?.content || ""}</p>
                      </div>
                    )}
                  </>
                )}
                  ...
                  <div className="bg-primary/10 rounded-lg p-8 border border-primary/20 mb-12">
                    <div className="flex flex-col md:flex-row items-start gap-6">
                      ...
                      <div>
                        ...
                        <div className="text-theme-body">
                          {communitySection?.description?.paragraphs
                            ? Object.keys(
                                communitySection.description.paragraphs
                              )
                                .sort()
                  ...
                  <>
                    ...
                    <div className="grid md:grid-cols-2 gap-8 mb-12">
                      ...
                      <div className="flex flex-col space-y-4">
                        {localKnowledgeSection?.reasons?.items &&
                        localKnowledgeSection.reasons.items.length > 0
                          ? localKnowledgeSection.reasons.items.map(
                              (reason, index) => (
                                <div
                                  key={index}
                                  className="flex items-start gap-3 bg-theme-bg-alt p-4 rounded-lg border border-theme-bg hover:shadow-md transition-all duration-300"
                                >
                                  {reason?.icon && (
                    ...
                    {localKnowledgeSection?.description?.paragraphs && (
                      <div className="text-theme-body mb-12">
                        {Object.keys(
                          ...
                          .map((key) => {
                            const paragraph =
                              localKnowledgeSection.description.paragraphs[key];
                            return paragraph ? (
                              <p key={key} className="mb-4 last:mb-0">
                                {paragraph?.content || ""}
                              </p>
                  ...
                  <div className="bg-theme-bg/30 rounded-lg p-8 mb-12">
                    <div className="flex flex-col md:flex-row items-start gap-6">
                      ...
                      <div>
                        ...
                        <div className="text-theme-body">
                          {personalApproachSection?.description?.paragraphs
                            ? Object.keys(
                                personalApproachSection.description.paragraphs
                              )
                                .sort()
                                .map((key) => {
                                  const paragraph =
                                    personalApproachSection.description
                                      .paragraphs[key];
                                  return paragraph ? (
                                    <p key={key} className="mb-4 last:mb-0">
                                      {paragraph?.content || ""}
                          ...
                          {personalApproachSection?.approaches?.items &&
                            personalApproachSection.approaches.items.length >
                              0 && (
                              <div className="grid md:grid-cols-3 gap-4 my-6">
                                {personalApproachSection.approaches.items.map(
                                  (approach, index) => (
                                    <div
                                      key={index}
                                      className="bg-white rounded-lg p-4 shadow-sm text-center border border-secondary/30"
                                    >
                          ...
                          {personalApproachSection?.note && (
                            <p>{personalApproachSection.note?.content || ""}</p>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-primary/5 rounded-lg p-8 border border-primary/10 mb-12">
                    <div className="flex flex-col items-center text-center">
                      ...
                      <div className="text-theme-body max-w-3xl mx-auto">
                        {thankYouSection?.description?.paragraphs
                          ? Object.keys(thankYouSection.description.paragraphs)
                              .sort()
                              .map((key) => {
                                const paragraph =
                                  thankYouSection.description.paragraphs[key];
                                return paragraph ? (
                                  <p key={key} className="mb-4 last:mb-0">
                                    {paragraph?.content || ""}
                                  </p>
