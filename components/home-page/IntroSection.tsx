...
async function getThemeData(): Promise<{
  aspectRatio: string;
  heroDividerSettings: HeroDividerSettings;
}> {
  const clientId = process.env.NEXT_PUBLIC_CLIENT_ID;
  ...
  return {
    ...
    heroDividerSettings:
      themeData.hero_divider_settings || DEFAULT_THEME.hero_divider_settings!,
  };
}
    ...
    <section className="py-20 bg-[#f5f5f5] relative w-full">
      ...
      <div className="container mx-auto px-4 py-4 max-w-screen-2xl">
        ...
        >
          ...
          {introContent?.image?.url && (
            <div
              className={`relative rounded-2xl overflow-hidden shadow-lg ${imageClasses} animate-fade-in-right`}
            >
              <Image
          ...
          <div className="animate-fade-in-left">
            ...
            {paragraphs?.map((paragraph, index) => (
              <p
                key={index}
                className={`text-secondary text-base md:text-lg lg:text-xl leading-relaxed ${index === paragraphs?.length - 1 ? "mb-8" : "mb-6"}`}
              >
                {paragraph?.content || ""}
              </p>
            ))}
