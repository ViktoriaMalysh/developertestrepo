      ...
      <div className="container mx-auto px-4 max-w-screen-xl">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 justify-items-center">
          {currentTopics.map((topic: any) => (
            <div
              key={topic.id}
              className="bg-card-bg rounded-xl shadow-lg border border-card-border hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden flex flex-col h-full"
            >
              <div
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <h2 className="text-xl font-heading font-semibold text-primary mb-2 hover:text-[#003366]">
                  <Link href={`${basePath}/${topic.slug}`}>{topic.name}</Link>
                </h2>
          ...
          {currentTopics.length === 0 && (
            <div className="col-span-full py-16 text-center">
              <p className="text-xl text-theme-body">
                No blog topics available at this time. Please check back soon.
              </p>
            </div>
          )}
        </div>
