      ...
      <div className="container mx-auto px-4 py-16 pt-12">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {badges.map((badge, idx) => (
                  <div
                    ...
                    style={{
                      backgroundColor:
                        "color-mix(in srgb, var(--divider-color) 20%, transparent)",
          ...
          <div>
            ...
            <ul className="space-y-3">
              ...
              {showCorePage && (
                <li>
                  <Link
                    href={locationPrefix ? `${locationPrefix}/about` : "/about"}
                    className="text-primary/80 hover:text-accent transition-colors link-underline"
                  >
                    About Us
                  </Link>
              ...
              {showCorePage && (
                <li>
                  <Link
                    href={locationPrefix ? `${locationPrefix}/faq` : "/faq"}
                    className="text-primary/80 hover:text-accent transition-colors link-underline"
                  >
                    FAQs
                  </Link>
              ...
              {showLocationDepedentPage && (
                <li>
                  <Link
                    href={locationPrefix ? `${locationPrefix}/blog` : "/blog"}
                    className="text-primary/80 hover:text-accent transition-colors link-underline"
                  >
                    Blog
                  </Link>
              ...
              {showCorePage && (
                <li>
                  <Link
                    href={
                      locationPrefix ? `${locationPrefix}/contact` : "/contact"
                    }
                    className="text-primary/80 hover:text-accent transition-colors link-underline"
                  >
                    Contact
              ...
              {socialLinksModalData &&
                socialLinksModalData.locations.length > 0 && (
                  <li>
                    <button
            ...
            {isMultiLocation && allLocations && allLocations.length > 0 ? (
                    ...
                    <div key={location.id} className="space-y-2">
                      ...
                      <ul className="space-y-2">
                        <li className="flex items-start">
                          <MapPin
                            size={16}
                            className="text-[var(--divider-color)] mt-1 mr-2 flex-shrink-0"
                          />
                          <span className="text-primary/80 text-sm">
                            ...
                            <span className="capitalize">
                              {location.city}
                            </span>, {location.state} {location.zip}
                          </span>
                        </li>
                        {location.phone && (
                          <li className="flex items-center">
                            <Phone
                              size={16}
                              className="text-[var(--divider-color)] mr-2 flex-shrink-0"
                            />
                            <a
                              href={locPhoneHref}
                              className="text-primary/80 hover:text-accent transition-colors text-sm"
                            >
                              {locPhone}
                            </a>
                          </li>
              ...
              <ul className="space-y-4">
                <li className="flex items-start">
                  <MapPin
                    size={18}
                    className="text-secondary mt-1 mr-3 flex-shrink-0"
                  />
        ...
        <div className="mt-8 border-t border-primary/10 pt-8 text-sm text-primary/70">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <p>
              &copy; {currentYear || "2025"}{" "}
              <span className="capitalize">{agencyName || ""}</span>. All rights
              reserved.
            </p>
          </div>
        </div>
